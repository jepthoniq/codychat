...
<?php } ?>
<?php if (loadAddonName('lock_rooms')) { ?>
    <?php if ($boom['user_iroom'] == 1 && boomAllow($data['custom2'])) { ?>
        <div onclick="allowloroom(<?php echo $boom['user_id']; ?>);" class="sub_list_item">
            <div class="sub_list_icon"><i class="fa fa-home default_color"></i></div>
            <div class="sub_list_content">Release the member from prison</div>
        </div>
    <?php } ?>
<?php } ?>
<?php if (loadAddonName('finger_print')) { ?>
    <?php if (boomAllow(9)) { ?>
        <div data="<?php echo $boom['user_id']; ?>" onclick="banFingerPrint(this);" class="sub_list_item">
            <div class="sub_list_icon"><i class="fa fa-ban error"></i></div>
            <div class="sub_list_content">blocking cemetery</div>
        </div>
    <?php } ?>
<?php } ?>