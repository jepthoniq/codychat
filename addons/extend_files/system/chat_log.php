...
require_once('addons_function.php');
...
if(isset($_POST['last'], $_POST['snum'], $_POST['caction'], $_POST['fload'], $_POST['preload'], $_POST['priv'], $_POST['lastp'], $_POST['pcount'], $_POST['room'], $_POST['notify'])){
	...
	if($fload == 0){
		...
		,boom_users.fancy_name, boom_users.name_wing1, boom_users.name_wing2, boom_users.name_smile, boom_users.user_level, boom_users.pic_shadow, boom_users.photo_frame, boom_users.user_badge, boom_users.another_prank, boom_users.name_glow, boom_users.sp_bg, boom_users.sp_bg_width, boom_users.prim_plus
		...
		if($log->num_rows > 0){
			while ($chat = $log->fetch_assoc()){
				...
				if($chat['snum'] != $snum || $ssnum == 1){
					$d['mlogs'] .= exCreateLogs($data, $chat, $ignore);
				}
			}
		}
	}
