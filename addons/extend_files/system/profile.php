...
<div <?php echo setProfileShadows($user); ?>>
	<div data-id="<?php echo $user['user_id']; ?>" class="modal_wrap_top modal_top profile_background <?php echo coverClass($user); ?>" <?php echo getCover($user); ?>>
		<div class="brow">
			<div class="modal_top_menu">
				...
				<?php if (loadAddonName('extend_files') && !empty($user['pro_phone']) || !empty($user['pro_wp']) || !empty($user['pro_fb']) || !empty($user['pro_insta']) || !empty($user['pro_tw'])) { ?>
			...
			<div class="bcell_bottom profile_top">
				<div class="btable_auto">
					<div id="proav" class="profile_avatar" data="<?php echo $user['user_tumb']; ?>">
						<div <?php if (!empty($user['photo_frame'])) { ?> style="position: relative;bottom: 0px;left: -2px;" <?php } ?>>
							...
							<?php if (!empty($user['photo_frame'])) { ?>
								<img class="over3" src="addons/extend_files/files/frame/<?php echo $user['photo_frame']; ?>" />
							<?php } ?>
						</div>
					...
					<div <?php if (!empty($user['photo_frame'])) { ?> style="padding: 0 25px;" <?php } ?> class="profile_tinfo cover_text">
						<?php if (loadAddonName('extend_files') && $user['user_prim'] > 0 && $user['prim_plus'] == 0) { ?>
							<div class="pdetails">
								<p class="cover_text bellips pro_rank" style="color: orange;"><img style="height:30px;width:auto;" src="addons/extend_files/files/icons/prim.png" /></p>
							</div>
						<?php } ?>
						<?php if (loadAddonName('extend_files') && $user['user_prim'] >= 0 && $user['prim_plus'] > 0) { ?>
							<div class="pdetails">
								<p class="cover_text bellips pro_rank" style="color: orange;"><img style="height:30px;width:auto;" src="addons/extend_files/files/icons/prim_plus.png" /></p>
							</div>
						<?php } ?>
						<div class="pdetails">
						...
						<div class="pdetails">
							<div class="bcell">
								<?php if (loadAddonName('profile_like') && boomAllow(1)) { ?>
									<?php if (!isBot($user) && boomAllow(1) && canLike($user)) { ?>
										<a style="margin-top: 8px;" onclick="proLike(<?php echo $user['user_id']; ?>);" value="<?php echo $user['user_id']; ?>" title="Love it" class="btn btn-counter" data-count="<?php echo $user['profile_like']; ?>"><span>❤</span>Like</a>
									<?php } ?>
									<?php if (!isBot($user) && boomAllow(1) && canUnLike($user)) { ?>
										<a style="margin-top: 8px;background: #f64136;color: white;" onclick="proUnLike(<?php echo $user['user_id']; ?>);" value="<?php echo $user['user_id']; ?>" title="Love it" class="btn btn-counter" data-count="<?php echo $user['profile_like']; ?>"><span style="color:white;text-shadow:none;">❤</span>Unlike</a>
									<?php } ?>
								<?php } ?>
								...
								<?php if (loadAddonName('users_badge') && boomAllow(11)) { ?>
									<button onclick="getUsersBadge(<?php echo $user['user_id']; ?>);" class="tiny_button ok_btn"><i class="fa fa-cc"></i>Nickname</button>
								<?php } ?>
							</div>
						</div>
					</div>
	...
	<?php } ?>
	<div id="ex_menu" class="modal_menu <?php if (loadAddonName('extend_files')) {
											echo $user['pro_color'];
										} ?>">
		<ul>
			...
			<?php } ?>
			<?php if (boomAllow(11) && loadAddonName('extend_files')) { ?>
				<li class="modal_menu_item" data="mprofilemenu" data-z="developer">Control</li>
			<?php } ?>
		</ul>
	</div>
	<?php if ($user['private_profile'] == 0) { ?>
		<div id="mprofilemenu" <?php if (loadAddonName('extend_files')) {
			...
			<div class="modal_zone pad25 tpad15" id="profile_info">
				<div class="clearbox">
					...
					<?php } ?>
					<?php if (loadAddonName('extend_files')) { ?>
						<div class="listing_half_element info_pro">
							...
							<div class="listing_text">
								...
								$upgrade = '';
								if (boomAllow(11)) {
									$upgrade = '<span onclick="upgradeToPremium(' . $user['user_id'] . ');" style="color:#d20343;border: 1px solid;border-radius: 3px;padding: 0px 3px;"><i style="height:15px;" class="fa fa-arrow-circle-up error"></i> ترقية</span>';
								}
						...
						<?php if ($user['sup_end'] > 0) { ?>
							<div class="listing_half_element info_pro">
								<div class="listing_title">Management membership expiration date</div>
						...
						<div class="listing_half_element info_pro">
							<div class="listing_title">Member level</div>
						...
						<div class="listing_half_element info_pro">
							<div class="listing_title">Left for the next level</div>
...
<p class="label">Modify a member's level</p>
	...
	<?php if ($user['private_profile'] == 1) { ?>
		<div class="empty_zone">
			<img class="empty_zone_icon" src="addons/extend_files/files/folder.svg">
			<p class="empty_zone_text sub_text">This profile is private</p>
			<p class="empty_zone_text sub_text">You cannot view this profile</p>
			<?php if (boomAllow(9)) { ?>
				<p class="empty_zone_text sub_text"><button data="<?php echo $user['user_id']; ?>" id="open_private_pro" class="tiny_button ok_btn"><i class="fa fa-unlock"></i> open file</button></p>
			<?php } ?>
		</div>
	<?php } ?>
	<?php if ($user['private_profile'] == 1) { ?>
		<img style="position: relative;bottom: -3px;width: 100%;" src="addons/extend_files/files/names/afl.png" />
	<?php } ?>
</div>
<?php if (!empty($user['pro_song']) && loadAddonName('extend_files')) { ?>
	<audio autoplay="true" loop="true" src="addons/extend_files/system/upload/<?php echo $user['pro_song']; ?>"></audio>
<?php } ?>
<?php if (!empty($user['pro_color']) && loadAddonName('extend_files')) { ?>
	<script>
...
<?php if (!empty($user['pro_text_main'])) { ?>
	<script>
		...
		$('.modal_menu').css('color', mainTextCol);
	</script>
<?php } ?>
...
<script src="addons/extend_files/js/custom.js"></script>